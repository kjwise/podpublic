(() => {
    ...
    const navigationPath = (url) => new URL(url, document.baseURI).pathname
        .replace(/\/index\.html$/, '/').replace(/\.html$/, '').replace(/\/+$/, '');
    const currentNavigationPath = navigationPath(window.location.href);
    for (const nav of document.querySelectorAll('.pod-desktop-nav, .pod-mobile-drawer')) {
        const links = [...nav.querySelectorAll('a')];
        const exact = links.find(link => navigationPath(link.href) === currentNavigationPath);
        const section = currentNavigationPath.includes('/blogs/') ? 'blogs' :
            (document.querySelector('.chapter-nav') ? 'toc' : null);
        const active = exact || (section && links.find(link =>
            navigationPath(link.href).endsWith(`/${section}`)));
        if (active) {
            active.setAttribute('aria-current', exact ? 'page' : 'location');
        }
    }
    ...
    if (chapterContent && !isTocPage && !isSearchPage) {
        ...
        const isStructuralBoundary = (element) =>
            ...
            !!(element.classList && element.classList.contains('poetry')) ||
    ...
    const positionSideNavAboveShare = () => {
        const ceiling = sharePanel ? sharePanel.getBoundingClientRect().top - 12 : Infinity;
        for (const link of [sideNavPrev, sideNavNext]) {
            if (!link || link.style.display === 'none') {
                continue;
            }
            const height = Math.max(link.offsetHeight,
                link.querySelector('.side-nav-chip')?.offsetHeight || 0);
            link.style.top = `${Math.min(window.innerHeight / 2, ceiling - height / 2)}px`;
        }
    };
    ...
    const positionSideNavLinks = () => {
        ...
        const getReadingRect = () => {
            ...
            for (const candidate of contentEl.querySelectorAll(selectors.join(', '))) {
            ...
            return right > left ? { left, right } : contentRect;
        };
    ...
    if (sideNavPrev || sideNavNext) {
        ...
        window.addEventListener('load', positionSideNavLinks);
        window.addEventListener('scroll', positionSideNavAboveShare, { passive: true });
        document.fonts.ready.then(positionSideNavLinks);
    }
})();
...
window.addEventListener('load', () => {
    ...
    // only activates for pointer devices at desktop widths.
    if (prefersFinePointer && !prefersReducedMotion) {
        ...
        if (mainEl && headerEl && contentEl) {
            ...
            const syncThreadViewport = () => {
                ...
                lastPointerY = null;
                currentState.opacity = 0;
                targetState.opacity = 0;
                writeThreadState();
            };

            desktopViewport.addEventListener('change', syncThreadViewport);
            syncThreadViewport();
        }
    }
}, { once: true });
